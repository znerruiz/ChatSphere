...
import Icon from 'react-native-vector-icons/Ionicons'; // For icons in the menu
...
const Menu = ({ navigation }) => {
  ...
  const menuItems = [
    {
      id: 'profile',
      title: 'Profile',
      icon: 'person-outline', // Ionicons icon
      navigateTo: 'ProfileScreen', // Your navigation route
    },
    {
      id: 'friends',
      title: 'Friends',
      icon: 'people-outline',
      navigateTo: 'FriendsScreen',
    },
    {
      id: 'settings',
      title: 'Settings',
    ...
    },
    {
      ...
      action: () => handleLogout(), // Trigger custom action like logging out
    },
  ];

  // Example action for logout
  const handleLogout = () => {
  ...
  const renderMenuItem = ({ item }) => (
    <TouchableOpacity
      onPress={() =>
        item.action ? item.action() : navigation.navigate(item.navigateTo)
      }
      className="flex-row items-center py-4 px-5 bg-primary mb-2 rounded-lg"
    >
      <Icon name={item.icon} size={24} color="gray" />
      <Text className="ml-4 text-lg text-white">{item.title}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-lightpink-100">
      ...
      <View className="p-4 bg-primary flex-row justify-between items-center">
        <Text className="text-white text-2xl font-fsemibold">Menu</Text>
        <Image
